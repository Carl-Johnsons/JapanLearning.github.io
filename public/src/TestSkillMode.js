import ComboBox from "./CustomCombobox.js";
// DECLARATION
const ModeComboBox = new ComboBox();
ModeComboBox.init();
...
async function handleScoreResults() {
    ...
    for (let i = 0; i < Question.length; i++) {
        if (Answer[i].trim().toLowerCase() === input[i].value.toLowerCase().trim() || AnswerHiragana[i].trim() === input[i].value.trim()) {
            score++;
